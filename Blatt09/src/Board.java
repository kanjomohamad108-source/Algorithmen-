...
public class Board {
    ...
    public int manhattan() {
        // TODO 1.1 manhattan
        int dist = 0;
        for (Position pos : positions) { //alle Positionen
            int value = getField(pos);          // Hole den Wert an der aktuellen
            if (value == BLANK) continue; // Das leere Feld (0) wird nicht berücksichtigt

            int targetX = (value - 1) % nx; // x-Koordinate im Zielzustand
            int targetY = (value - 1) / nx; // y-Koordinate im Zielzustand

            dist += Math.abs(pos.x - targetX) + Math.abs(pos.y - targetY);
        }
        return dist;
    }
