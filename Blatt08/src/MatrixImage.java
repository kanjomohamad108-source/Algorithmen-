...
public class MatrixImage implements Image {
    ...
    */
    ...
    public double contrast(Coordinate p0, Coordinate p1) throws InputMismatchException {
        // TODO
        // Prüfe: Liegen die im gültigen Bereich des Bildes?
        if (p0.x < 0 || p0.x >= sizeX() || p0.y < 0 || p0.y >= sizeY() ||
                p1.x < 0 || p1.x >= sizeX() || p1.y < 0 || p1.y >= sizeY()) {
            throw new InputMismatchException("Coordinates must be between 0 and " + (sizeX() - 1) + " and " + sizeY());
        }
        // hole dir die zeilen
        int value1 = field[p0.x][p0.y];
        int value2 = field[p1.x][p1.y];

        // Berechne den absoluten Unterschied (|value1 - value2|),
    ...
    public void removeVPath(int[] path) {
        // TODO
        if (path.length != sizeY()) {
            throw new IllegalArgumentException("Pfadlänge passt nicht zur Bildhöhe");
            }

        // kopieren und Neue Matrix mit 1 Spalte weniger
        MatrixImage name = new MatrixImage(sizeX() - 1, sizeY());
        for (int y = 0; y < sizeY(); y++) {
            int newX = 0;               // Spaltenindex in neuer Matrix
            int skipX = path[y];        // Diese Spalte überspringen
            for (int x = 0; x < sizeX(); x++) {
                if (x == skipX) continue; // Diesen Wert nicht kopieren
                name.field[newX][y] = field[x][y];
                newX++;
                }
            }
            // Setze neue Matrix als aktuelles Bild
