...
public class DepthFirstPaths {
    ...
    public List<Integer> pathTo(int v) {
        // TODO
        // Überprüft, ob der Knoten v im gültigen Wertebereich liegt (0 ≤ v < Anzahl der Knoten)
        validateVertex(v);
        // Wenn der Knoten v während der Tiefensuche nicht erreicht wurde, gibt es keinen Pfad von s nach v
        if (!marked[v]) return null;
        // Erstelle eine neue leere Liste, in der der Pfad gespeichert wird
        LinkedList<Integer> path = new LinkedList<>();
        // Rückverfolgung vom Zielknoten v zurück zum Startknoten s mithilfe von edgeTo[]
        // Dabei wird jeder besuchte Knoten am Ende der Liste angehängt
        for (int x = v; x != s; x = edgeTo[x]) {
            path.addLast(x);  // addLast fügt hinten ein → Pfad ist rückwärts sortiert
        }
        // Füge den Startknoten s ebenfalls ans Ende der Liste hinzu
        path.addLast(s);
        // Gib die Pfadliste zurück (von Ziel zu Start, nicht wie erwartet von Start zu Ziel)
        return path;
    }
